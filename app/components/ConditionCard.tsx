...
import { DrupalCondition } from '@/lib/types'
import ResponsiveImage from './ResponsiveImage'
import { ArrowRight } from 'lucide-react'

interface ConditionCardProps {
  item: DrupalCondition
}

export default function ConditionCard({ item }: ConditionCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).treatmentApproach && (
            <p className="text-sm text-teal-700 font-medium mb-2">{(item as any).treatmentApproach}</p>
          )}
